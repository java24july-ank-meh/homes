...
angular.module("app", ['angularjs-dropdown-multiselect']).controller("resident", function($scope,$http){
  ...
  $scope.group = "0";
    ...
    self.onload = function() {
    	$http.post("/resident/message/listusers",{complex:"general"}).then(function(response){
	...
	self.messageUser = function(){
		...
		$http.post(url, {complex:self.userMessageComplex, unit:self.userMessageUnit, message:self.userMessageToSend, ids:$scope.dataModel})
				.success(function(response){
					console.log(response);
				}).error(function(response){
					console.log(response);
	...
	self.DirectMessageUser = function(){
		alert($scope.group);
		alert($scope.directModel);
		let url = "/resident/message/users/direct";
		$http.post(url, {complex:self.DirectMessageComplex, unit:self.DirectMessageUnit, message:self.DirectMessageToSend,group:$scope.group, ids:$scope.directModel})
				.success(function(response){
					console.log(response);
				}).error(function(response){
					console.log(response);
...
function ($filter, $document, $compile, $parse) {
    ...
    return {
        ...
        template: function (element, attrs) {
            ...
            if (checkboxes) {
                template += '<div class="checkbox"><name><input class="checkboxInput" type="checkbox" ng-click="checkboxClick($event, getPropertyForObject(option,settings.idProp))" ng-checked="isChecked(getPropertyForObject(option,settings.idProp))" /> {{getPropertyForObject(option, settings.displayProp)}}</name></div></a>';
            } else {
